...
export class RulesComponent implements OnInit {
  ...
  constructor(private browserService: BrowserService, private fb: FormBuilder, route: ActivatedRoute, private articleParserService: ArticleParserService) {
    // TODO 符合 url 的规则排在上面
  }
  ...
  ruleToForm = (data: Rule) => this.fb.group({
    patterns: data.patterns?.map(it => this.fb.control(it)).reduce((prev, curr) => {
      prev.push(curr)
      return prev
    }, this.fb.array([])) ?? this.fb.array([]),
    selector: this.fb.control(data.selector),
    unwanted: data.unwanted?.map(it => this.fb.control(it)).reduce((prev, curr) => {
      prev.push(curr)
      return prev
    }, this.fb.array([])) ?? this.fb.array([])
  })
